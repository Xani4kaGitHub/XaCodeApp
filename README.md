...
## Выпуск обновления через GitHub

Автообновление использует публичные GitHub Releases репозитория `Xani4kaGitHub/XaCodeApp`. Для выпуска новой версии:

1. Измените поле `version` в `package.json`, например на `1.11.2`.
2. Закоммитьте и отправьте изменения в ветку `main`.
3. Создайте и отправьте тег с той же версией:
...
GitHub Actions проверит проект, соберёт Windows-установщик и опубликует Release вместе с файлами `latest.yml` и `.blockmap`. Установленное приложение автоматически проверяет этот Release. После загрузки обновления кнопка в разделе «Настройки → Приложение» перезапускает XaCode и запускает установку.
